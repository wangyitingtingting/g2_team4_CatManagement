...
<div class="wrap">
    ...
    <div id="customConfirm">
        <button class="close-btn" onclick="closeConfirm()">X</button>
        <p>是否领养？</p>
        <button onclick="confirmResult(1)">是</button>
        <button onclick="confirmResult(0)">否</button>
    </div>
...
<div id="adoptionSection" class="admin">
    <h2>申请领养列表</h2>
    <table>
        <thead>
        <tr>
        ...
        <tbody>
        <c:forEach var="adoption" items="${requestScope.adoptions}">
            <tr>
                <td>${adoption.animalName}</td>
                <td>${adoption.userName}</td>
                <td>${adoption.experience}</td>
                <td>${adoption.reason}</td>
                <td>${adoption.approved}</td>
                <td>
                    <button type="button" onclick="approveAdoption('${adoption.animalName}', '${adoption.reason}', '${adoption.approved}')">领养</button>
                </td>
            </tr>
        </c:forEach>
        </tbody>
...
<script>
    ...
    function del(id) {
        $.ajax({
            url: '/PetAdoptionManagementSystem/adminDel',
            method: "post",
            data: { 'id': id},
    ...
    function update(id) {
        $.ajax({
            url: '/PetAdoptionManagementSystem/getUpdateAdmin',
            method: "post",
            data: { 'id': id},
            success: function (data) {
                data = JSON.parse(data);
    ...
    function deluser(id) {
        $.ajax({
            url: '/PetAdoptionManagementSystem/userDel',
            method: "post",
            data: { 'id': id},
            success: function (data) {
                data = JSON.parse(data);
    ...
    // 用户点击“是”
    function confirmResult(flag) {
        document.getElementById('customConfirm').style.display = 'none';

        $.ajax({
            url: '/PetAdoptionManagementSystem/adoptionAction',
            method: "post",
            data: { 'animalName': animalName, 'action': flag , 'reason': reason},
            success: function () {
                location.href = "/PetAdoptionManagementSystem/adoptionManage";
            }
        })
    ...
    function approveAdoption(animalNameTemp, reasonTemp, approvedTemp) {
        animalName = animalNameTemp
        approved = approvedTemp
        reason = reasonTemp;
        if(!!approved){
           alert("不可重复申请");
           return;
